...
func updateWallet(wlt *KeystoreWallet, password, newPassword string) error {
	...
	err = util.CopyAllFiles(wlt.dirName, tempDir)
	if err != nil {
		return err
	}

	for _, acc := range wlt.Accounts() {
		err = wlt.Update(acc, password, newPassword)
		if err != nil {
			break
		}
	}
	...
	if err != nil {
		files, err := ioutil.ReadDir(wlt.dirName)
		if err != nil {
			return err
		}
		for _, file := range files {
			...
			os.Remove(filepath.Join(wlt.dirName, file.Name()))
		}

		util.CopyAllFiles(tempDir, wlt.dirName)
	}
	return err
